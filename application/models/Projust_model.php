...
class Projust_model extends CI_Model{
	public function tambah(){
		...
		if($upload_foto_paspor){
				...
				$config['upload_path'] = './assets/berkas/projustisia/';
		...
		$this->db->insert('projustisia', $data);
	}

	public function update(){
		$id_projus=$this->input->post('id_projus',true);
		$data['projustisia']=$this->db->get_where('projustisia',['id_projus' => $id_projus])->row_array();
		...
		if($upload_foto_paspor){
				...
				if($this->upload->do_upload('foto_paspor')){
					...
					$old_foto_paspor = $data['projustisia']['foto_paspor'];
					
					unlink(FCPATH . 'assets/berkas/projustisia' . $old_foto_paspor);
			...
			if($upload_foto_wna){
				...
				if($this->upload->do_upload('foto_wna')){
					...
					$old_foto_wna = $data['projustisia']['foto_wna'];
					
					unlink(FCPATH . 'assets/berkas/projustisia' . $old_foto_wna);
